...
The previous chapter focused on how to allocate capital across assets. This chapter asks a complementary question: once a portfolio exists, how large could a short-run market loss be? Value at Risk (VaR) provides a compact answer by combining a time horizon, a confidence level, portfolio weights, and a loss distribution.

If $L$ is the portfolio loss over the chosen horizon and $\alpha$ is the confidence level, VaR is the loss threshold such that losses exceed it with probability $1-\alpha$:

$$
P(L > VaR_\alpha)=1-\alpha.
$$

Equivalently, when losses are sorted from low to high:
...
Inspect the first rows of the database.
...
The following is the US dollar equivalent of stock indices for historical simulation. This is the same as @Hull, Table 22.2:
...
```
The VaR estimation uses 501 observations because we count from day zero through day 500. The *A* before the index name stands for *adjusted*. A graphical representation of the table above:
...
```
The time-series starts on Monday August 7, 2006 and ends on Thursday September 25, 2008. These are 501 daily past observations. In the plot below, we also show the value of Friday September 26, 2008 as a dot, this is observation 502 and we call it <tt>`tomorrow`</tt>. This observation 502 is excluded from the VaR estimation because it represents an unknown future value, and it will be useful for us to evaluate our estimations.
...
This is similar to Table 22.3 in @Hull. The next plot shows the scenarios together with the actual values for September 26, 2008:
...
#| fig-cap: "Today is September 25, 2008. What is the worst that can happen to my portfolio tomorrow?"
plot(l, type = "h", lwd = 2,
...
The loss distribution can also be summarized with a histogram.

```{r}
#| label: fig-holftscbs2a22ah
#| fig-cap: "Histogram of losses for the scenarios considered between September 25 and 26, 2008: a histogram."
...
The point refers to what really happened, and the red line is the VaR following a historical approach. The same information can also be shown with a density plot.

```{r}
#| label: fig-holftscbs2a22adp
#| fig-cap: "Histogram of losses for the scenarios considered between September 25 and 26, 2008: a density plot."
...
#| fig-cap: "Figure 22.3. Histogram of losses for the scenarios considered between September 25 and 26, 2008."
...
#| fig-cap: "Extract of Table 22.4 in a graph: 1 percent historical VaR 253.385, realized loss in green."
...
The top 1% can also be explored with the <tt>`quantile()`</tt> function. R's default quantile estimator interpolates between order statistics, so its 99% value differs from Hull's fifth-worst-scenario historical VaR.
...
To reproduce Hull's discrete historical VaR, we can take the fifth largest loss directly:
...
If we use the default <tt>`quantile()`</tt> function at the 99% level we end up with:
...
The drawback of this method is that it assumes a normal distribution and we know $l$ is not normal. The advantage of this method is its simplicity.
...
This is slightly different from [@Hull]: 8,761.833. The standard deviation is the square root of this:
...
In @Hull, this value is \$217,757, which compares with \$253,385, calculated using the historical simulation approach.
...
We now implement this approach for the same portfolio problem. The covariance matrix on September 25, 2008 is calculated using the EWMA method with $\lambda=0.94$.
...
The next table replicates Table 23.8: volatilities (% per day) using equal weighting and EWMA.
...
The next step calculates optimal weights.
...
We now have four portfolio-weight alternatives, two of which are optimized. The risk-management question is whether optimized weights reduce VaR under the same data and horizon.
...
In this @Hull example we assume portfolio weights remains constant. However, we can rebalance portfolio weights. Here, we have the time evolution of minimum variance optimal weights when we consider 5 US industry portfolios as investment assets:
...
We return to the main @Hull example and compare the results graphically.

```{r}
#| label: fig-haistroiabaawner
#| fig-cap: "Hull's allocation is sub-optimal, the red one is a better alternative, although with negative expected return."
W_hull <- alpha / 10000
R_hull <- c(t(W_hull) %*% E)
...
```{r}
#| label: fig-mvraroib
#| fig-cap: "Minimum variance recommendation and realized outcome in blue."
set.seed(1)
...
```{r}
#| label: fig-hsoraro
#| fig-cap: "Hull sub-optimal recommendation and realized outcome."
set.seed(1)
...
```{r}
#| label: fig-craro
#| fig-cap: "CAC40 recommendation and realized outcome."
set.seed(1)
...
```{r}
#| label: fig-eoanoarce
#| fig-cap: "Estimation of a new optimal alpha: Replicate CAC40 ER."
plot(S, ER, type = "l", lwd = 3, xlim = c(0.7, 1.6),
...
As stated before, Hull's sub-optimal portfolio allocation turns out to be problematic. His recommendation would lead to a return of -0.558%, equivalent to a loss of 0.558%. The optimal minimum variance allocation would lead to a positive return of 0.2629%, and the optimal portfolio that targets the CAC 40 expected return leads to an even higher return of 0.518%.
...
We now return to the historical approach and incorporate the comparison with optimal weights.
...
As a reference, this is the @Hull original case.

```{r}
#| label: fig-lwhsadfcn
#| fig-cap: "Losses with Hull's suboptimal alpha DJ=0.4, FTSE=0.3, CAC=0.1, Nikkei=0.2."
...
The next calculation applies the historical approach with minimum-variance optimal weights.

```{r}
#| label: fig-lwhsadfcn2
#| fig-cap: "Losses with optimal alpha DJ=0.56, FTSE=0.038, CAC=0.022, Nikkei=0.38."
...
The range of losses and gains is now reduced. The one-day 99% Value at Risk can be estimated as the fifth-worst loss.
...
```{r}
#| label: fig-lwcoadfcn
#| fig-cap: "Losses with CAC40 optimal alpha DJ=0.607, FTSE=-0.403, CAC=0.459, Nikkei=0.336."
...
The range is reduced while still allowing larger gains in some scenarios. The one-day 99% Value at Risk can be estimated as the fifth-worst loss.
...
The three historical approaches can now be compared side by side.

```{r}
#| label: fig-hsaoaoa
#| fig-cap: "Hull's suboptimal alpha (left); optimal alpha (center); optimal alpha CAC40 (right)."
...
```{r}
#| label: fig-qcold
#| fig-cap: "Quantile comparison of losses distribution."
...
The chapter's main value is integration. The VaR number is only the final
summary. Before that summary, the analyst had to prepare international index
data, convert foreign values into US dollars, define portfolio weights,
generate loss scenarios, estimate volatility and covariance inputs, compare
allocation choices, and read the realized outcome. This is why VaR belongs in
the Financial Modeling with R path: it turns data, returns, allocation, and
market-risk assumptions into a single loss statement that can be audited and
recomputed.
