...
Add a box constraint such that the portfolio optimal weights are between 0 and 1.
...
The alternative of the box constraint is to allow the model to deliver negative portfolio weights. Negative portfolio weights represent short sales. Short selling is an investment strategy that speculates on the decline in a stock. Sometimes short selling is not allowed so the box constraint will deliver only positive portfolio weights. The <tt>`optimize.portfolio()`</tt> function allows us to solve our portfolio problem by taking a variety of methods, constraints and objectives. 
