...
```{r fig.cap="FANG, the four individual assets."}
...
```{r fig.cap="FANG, random method optimal portfolio."}
# Plot results.
...
```{r fig.cap="FANG, random method optimal weights."}
...
```{r fig.cap="FANG, ROI method optimal portfolio."}
# Plot results.
...
```{r fig.cap="FANG, ROI method optimal weights."}
...
FANG_stats |>
  ...
  arrange(-sr)
```
It may be disappointing to find out that the naïve portfolio has the higher Sharpe ratio including the optimal alternatives. We can add a target return constraint to the optimization problem so we can find out an optimal portfolio that targets the naïve portfolio expected return.
...
```{r fig.cap="FANG, ROI method optimal weights that targets the naïve portfolio expected return."}
...
```{r fig.cap="FANG, ROI method that targets the naïve portfolio expected return."}
# Plot results.
...
```{r fig.cap="FANG, simulated returns of investment alternatives."}
# The boxplot.
...
```{r fig.cap="FANG, simulated returns of investment alternatives. What really happened by 2017-01-30 in red."}
# The simulation.
boxplot(b, las = 2)
abline(h=0, lty = 2)
...
```{r fig.cap="10-assets, random method optimal portfolio."}
set.seed(13)
...
```{r fig.cap="10-assets, random method optimal weights."}
chart.Weights(opt_rand)
```

Finally the ROI method.

```{r fig.cap="10-assets, ROI method optimal portfolio."}
...
```{r fig.cap="10-assets, ROI method optimal weights."}
chart.Weights(opt_roi)
```
...
```{r fig.cap="2-assets, efficient frontier."}
...
```{r fig.cap="2-assets, efficient frontier and minimum variance portfolio."}
plot(sd_xy, ret_xy, type = "l", xlim = c(0, 0.15), ylim = c(0, 0.05))
abline(v = 0, lty = 2)
abline(h = 0, lty = 2)
points(sd(fang_xy$X), mean(fang_xy$X), cex = 2, pch = 19, col = "red")
...
```{r fig.cap="FANG plus X and Y, random portfolio."}
# Plot results.
...
```{r fig.cap="Indexes, optimal weights."}
...
```{r fig.cap="Indexes, portfolio performance."}
