...
It is implemented in R package \texttt{digest}.
...
If you are interested in comparing passwordsgenerator.net with \texttt{digest} in R, you should add \texttt{serialize = FALSE}.

```{r}
digest("A blockchain is a chain of blocks", "sha256", serialize = FALSE)
```
...
```
It takes 165 iterations to solve the problem of difficulty 2. How difficult is difficulty 3? This time \texttt{print = FALSE} to save space.
...
Let’s create a pool of fictitious pending transactions. We store them in a data frame structure using the \texttt{tibble} package.
...
Next is an example of encryption and digital signature using package \texttt{openssl}. First, the encryption example.
...
The \texttt{charToRaw} converts a length-one character string to raw bytes.
...
data
```
In computing, serialization is the process of translating a data structure or object state into a format that can be stored or transmitted and reconstructed later. Here, the transaction is translated into a format that can be reconstructed later. At the moment, this object called \texttt{data} has not been signed yet by the sender. The next code chunk sign the \texttt{data} object by the sender.
...
sig
```
The new object \texttt{sig} has not only the translated transaction, it has been also signed by the sender. With the function \texttt{signature\_verify}, anyone could verify that the sender of the transaction is the owner of the \texttt{pubkey}. 
...
Anyone could know that the transaction belongs to the sender with the \texttt{pubkey}. Now, we encrypt the translated and signed transaction.
...
ciphermsg
```
The transaction is translated, signed by the sender and now encrypted with the sender's \texttt{pubkey}. To recover the original transaction we would need to decrypt the serialized message with the private key \texttt{key}, and then unserialize the result.
