...
```{r fig.cap="Apple stock prices, improved format."}
plot(aapl_stock_prices$date, aapl_stock_prices$adjusted, 
...
```{r fig.cap="Apple stock prices and SP500."}
par(mar = c(5, 5, 2, 5))
...
```{r fig.cap="Apple stock trading volume."}
...
The output above shows when (in the variable <tt>`when`</tt>) Apple stock had its highest price change (<tt>`top`</tt>) in one day.

```{r fig.cap="Apple stock trading volume a further analysis."}
...
The <tt>`quantmod`</tt> package can retrieve exchange rates easily.

```{r fig.cap="USD/MXN exchange rate."}
# Load the package.
library(quantmod) 
...
plot(lag(exchange_rate$USD.MXN), exchange_rate$USD.MXN)
ex <- lm(exchange_rate$USD.MXN ~ lag(exchange_rate$USD.MXN))
summary(ex)
```
...
```{r PMF_EF}
exchange_rate <- as.data.frame(exchange_rate)
ch_ex <- (exchange_rate - lag(exchange_rate))/lag(exchange_rate)
ex_new <- data.frame(ch_ex, exchange_rate$USD.MXN)
ex_new[ex_new$USD.MXN>0.01 | ex_new$USD.MXN < -0.01,]
```

```{r eval=FALSE, include=FALSE}
exchange_rate <- getSymbols("USD/MXN", src = "oanda", auto.assign = FALSE)
exchange <- data.frame(exchange_rate)
R_USDMXN <- exchange |> 
  tq_transmute(select=USD.MXN, mutate_fun=periodReturn,
               period="daily", type="arithmetic")
subset(R_USDMXN, USD.MXN_returns > 0.01 | USD.MXN_returns < -0.01)
```


```{r PMF_E1 3}
exchange_rate <- getSymbols("USD/MXN", src = "oanda", auto.assign = FALSE)
sum(exchange_rate > 20.6) / length(exchange_rate)
```

```{r PMF_E1 4}
exchange_rate <- getSymbols("USD/MXN", src = "oanda", auto.assign = FALSE)
exchange_rate[which.max(exchange_rate)]
exchange_rate[which.min(exchange_rate)]
```

Here, 1 USD equals `r last(exchange_rate)` MXN as `r Sys.Date()`. We can show the time series information into a density plot.

```{r fig.cap="USD/MXN exchange rate, a density plot."}
# Density plots.
...
```{r fig.cap="Apple Candlestick chart."}
# Plot the data.
AAPL |>
    ggplot(aes(x = date, y = close)) +
    geom_candlestick(aes(open = open, high = high, low = low, 
                         close = close)) +
    labs(y = "Closing Price", x = "") + theme_tq()
```
...
```{r fig.cap="Apple Candlestick chart, zoom."}
# A candlestick plot.
AAPL |>
    ggplot(aes(x = date, y = close)) +
    geom_candlestick(aes(open = open, high = high, low = low, 
...
```{r fig.cap="Apple Moving average chart."}
...
```{r fig.cap="Apple Bollinger bands and simple moving average."}
AAPL |>
...
```{r fig.cap="Facebook Moving average chart."}
# The simple moving average (SMA) plot. 
FANG |>
  filter(symbol == "FB") |>
  ggplot(aes(x = date, y = close)) +
