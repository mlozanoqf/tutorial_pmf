...
The previous chapters stayed close to market data, trading, asset pricing,
portfolio allocation, and portfolio risk. This final technical chapter changes
the object of analysis. Blockchain is presented as a special application of
financial modeling ideas to distributed records, transaction validation,
cryptographic signatures, and incentives. It extends the toolkit as a special
application at the end of the book.
...
blockchain[[2]]
```
Let’s also write a validation function for the blockchain. For now, we only check that the parent field of a non-genesis block references to the previous block in the chain. Therefore, we validate a blockchain as long as its sequence is correct. This validation rule is simple and enough for the toy example. For example, first the genesis, then the first one, then the second, and finally the third.
...
Hashes of blocks are created using cryptographic hash functions, that are mathematical algorithms that maps data of arbitrary size to a bit string of a fixed size (called hash or digest). The hash is used to certify the information content of the block: by modifying even a single bit of the content, the hash completely changes. Furthermore, notice that the digest of the current block is computed in terms of the previous block digest. This means if you alter one block you need to modify its hash and the hash of every following block for the chain to be valid.
...
The hash of block 3 depends on the block 3 information and the block 2 hash, which at the same time depends on the block 2 information and the block 1 hash. The blocks are chained.
...
Digital signatures are usually applied to a hash of the message. The hash keeps the signed object short while preserving integrity: any change to the original message changes the hash and invalidates the signature. Confidentiality is a separate layer. A sender can encrypt the message, often through hybrid or envelope encryption, using the receiver's public key or a symmetric key protected by that public key. The receiver verifies the signature with the sender's public key and decrypts with the appropriate private key.
...
sig
```
The new object <tt>`sig`</tt> is the digital signature of the serialized transaction. With the function <tt>`signature_verify()`</tt>, anyone can verify that the serialized transaction was signed with the private key associated with <tt>`pubkey`</tt>.
...
Verification confirms that the holder of the private key associated with <tt>`pubkey`</tt> signed this serialized transaction. Now, we encrypt the translated transaction.
...
ciphermsg
```
The transaction data is serialized and encrypted in this toy example with <tt>`pubkey`</tt>. In a real confidential transfer, encryption would use the receiver's public key or a symmetric key protected for the receiver. The signature remains a separate object used for authenticity and integrity. To recover the original transaction we would need to decrypt the serialized message with the private key <tt>`key`</tt>, and then unserialize the result.
